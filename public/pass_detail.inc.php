...
//     }
    ...
    for($i=0;$i<$date_len;$i++) {
        ...
        if(!mysqli_stmt_prepare($stmt,$sql)) {
            ...
            mysqli_stmt_bind_param($stmt,'iissssi',$_SESSION['userId'],
                ...
                $_POST['midname'][$i],$_POST['lastname'][$i],$_POST['flight_id']);                           
             mysqli_stmt_execute($stmt);  
            $flag = true;        
        }
    }   
    if($flag) {
        ...
        //  header('Location:');
        $_SESSION['userUid']=$_POST['userUid'];
        header('location: ./payments/payments/index.php? fname= '.$_SESSION['userUid']."lname= ".$_SESSION['lname'] );
        exit();          
    }
